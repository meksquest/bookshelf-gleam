...
// build/dev/javascript/bookshelf/bookshelf.mjs
var FILEPATH = "src/bookshelf.gleam";
var Model = class extends CustomType {
  constructor(total, cats) {
    super();
    this.total = total;
    this.cats = cats;
  }
...
var UserClickedAddCat = class extends CustomType {
};
var UserClickedRemoveCat = class extends CustomType {
};
  ...
  let decoder = field(
    ...
    (id) => {
      return field(
        ...
        string2,
        (url2) => {
          return success(new Cat(id, url2));
        }
      );
    }
  );
...
function update2(model, msg) {
  if (msg instanceof UserClickedAddCat) {
    return [
      (() => {
        let _record = model;
        return new Model(model.total + 1, _record.cats);
      })(),
      get_cat()
    ];
  } else if (msg instanceof UserClickedRemoveCat) {
    return [
      (() => {
        let _record = model;
        return new Model(model.total - 1, drop(model.cats, 1));
      })(),
      none()
    ];
  } else {
    ...
    if ($ instanceof Ok) {
      ...
      return [
        (() => {
          let _record = model;
          return new Model(_record.total, append(model.cats, cats));
        })(),
        none()
      ];
    } else {
  ...
  return div(
    ...
    toList([
      div(
        ...
        toList([
          button(
            toList([on_click(new UserClickedAddCat())]),
            toList([text3("Add cat")])
          ),
          p(toList([]), toList([text3(to_string(model.total))])),
          button(
            toList([on_click(new UserClickedRemoveCat())]),
            toList([text3("Remove cat")])
          )
        ])
      ),
      div(
